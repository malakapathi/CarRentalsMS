...
if (isset($_POST['Update_Staff_Profile_Photo'])) {
    // Ensure user_dpic field exists in $_FILES and $_POST
    if (isset($_FILES['user_dpic']) && isset($_FILES['user_dpic']['name']) && isset($_POST['user_id'])) {
        // File details
        $user_dpic_name = $_FILES['user_dpic']['name'];
        $user_id = mysqli_real_escape_string($mysqli, $_POST['user_id']);

        // Allowed image types
        $allowed_types = array('jpg', 'jpeg', 'png', 'gif');
        $file_extension = strtolower(pathinfo($user_dpic_name, PATHINFO_EXTENSION));

        // File size limit (5MB)
        $max_file_size = 5 * 1024 * 1024; // 5MB

        // Check file type and size
        if (in_array($file_extension, $allowed_types) && $_FILES['user_dpic']['size'] <= $max_file_size) {
            // Generate a unique filename
            $new_user_dpic = 'Car_Rental_Staff_DPIC_' . time() . '.' . $file_extension;

            // Move the uploaded file to the storage directory
            $storage_directory = '../storage/users/';
            $destination = $storage_directory . $new_user_dpic;

            // Move uploaded file securely
            if (move_uploaded_file($_FILES['user_dpic']['tmp_name'], $destination)) {
            ...
            $err = "Invalid file type or exceeds maximum file size limit";
        }
    } else {
